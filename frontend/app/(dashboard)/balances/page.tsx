...
import { ChevronDown, ChevronRight, HandCoins, Plus, Search } from 'lucide-react'
...
export default function BalancesPage() {
  ...
  const createBalance = useMutation({
    ...
    onSuccess: () => {
      for (const key of ['balances', 'receivables', 'collections-summary', 'collections-insights', 'client-summary']) queryClient.invalidateQueries({ queryKey: [key] })
  ...
  return <PageFrame>
    <PageHeader eyebrow="Cuenta corriente" title="Saldos" description="Deudas pendientes de cada cliente. Los pagos que registrás en Cobranzas las cancelan automáticamente, empezando por la más antigua de la misma moneda." action={canWrite ? <button type="button" className="btn-primary" onClick={() => setShowForm(true)}><Plus size={15} /> Cargar saldo</button> : undefined} />
        ...
        <tbody>{items.map((item) => {
          ...
          return <Fragment key={key}>
            <tr className="cursor-pointer hover:bg-[var(--paper-soft)]" onClick={(event) => { if (!(event.target as HTMLElement).closest('a, button')) setExpanded(open ? null : key) }}>
              ...
              {canWrite ? <td className="text-right">{item.company && !item.company.isArchived ? <Link className="btn-secondary whitespace-nowrap" href={`/collections?cobrar=${encodeURIComponent(item.companyId)}&moneda=${item.currency}`}><HandCoins size={14} /> Cobrar</Link> : null}</td> : null}
            </tr>
            {open ? <tr><td colSpan={canWrite ? 9 : 8} className="bg-[var(--paper-soft)] !p-0"><BalanceDetail balance={item} /></td></tr> : null}
          </Fragment>
        })}</tbody>
...
function BalanceDetail({ balance }: { balance: ClientBalance }) {
  ...
  return <div className="px-4 py-3"><table className="data-table"><thead><tr><th>Concepto</th><th>Vencimiento</th><th>Estado</th><th className="text-right">Importe</th><th className="text-right">Cobrado</th><th className="text-right">Saldo</th></tr></thead><tbody>{open.map((item) => <tr key={item.id}>
