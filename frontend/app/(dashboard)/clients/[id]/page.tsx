...
export default function ClientDetailPage() {
  ...
  return (
    <PageFrame>
      <PageHeader eyebrow="Legajo contable" title={client.name} description={client.legalName || 'Información fiscal y operativa consolidada.'} backHref="/clients" action={canClaim || canRelease || canWrite || canManage ? <div className="flex flex-wrap gap-2">{canClaim ? <button type="button" className="btn-primary" disabled={claimClient.isPending} onClick={() => claimClient.mutate()}><UserCheck size={16} />{claimClient.isPending ? 'Tomando…' : 'Tomar cliente'}</button> : null}{canRelease ? <button type="button" className="btn-secondary" disabled={releaseClient.isPending} onClick={() => releaseClient.mutate()}><UserMinus size={16} />{releaseClient.isPending ? 'Liberando…' : 'Liberar cliente'}</button> : null}{canManage ? <PaymentSummaryButton clientId={id} /> : null}{canWrite ? <button type="button" className="btn-secondary" onClick={startEditing}><Pencil size={16} />Editar legajo</button> : null}{canManage ? <button type="button" className="btn-danger" disabled={deleteClient.isPending} onClick={async () => { if (await confirmDelete(`El cliente "${client.name}" se enviará a la papelera. Sus pagos y cargos se conservan para mantener el historial contable.`)) deleteClient.mutate() }}>{deleteClient.isPending ? 'Eliminando…' : 'Eliminar cliente'}</button> : null}</div> : undefined} />
      ...
      {tab === 'cuenta' && canManage ? <PaymentSummaryButton clientId={id} /> : null}
